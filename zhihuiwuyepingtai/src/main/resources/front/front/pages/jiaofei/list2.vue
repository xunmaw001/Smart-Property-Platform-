	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('jiaofei','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                 ...
                 <view v-show="item.jiaofeiYuefen" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">缴费月份</text>
                     <text style="width: 70%">{{item.jiaofeiYuefen}}</text>
                 </view>
                 <view v-show="item.jiaofeiValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                 ...
                 <view v-show="item.jiaofeiShifouValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let jiaofeiTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'jiaofei_types'
		  	});
		  	this.jiaofeiTypesList = jiaofeiTypes.data.list;
			jiaofeiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
            this.categoryList = jiaofeiTypes.data.list

	   		//当前表的 是否缴费 字段 字典表查询方法
		  	let jiaofeiShifouTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'jiaofei_shifou_types'
		  	});
		  	this.jiaofeiShifouTypesList = jiaofeiShifouTypes.data.list;
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`jiaofei`, {
                                id:id,
                            });
