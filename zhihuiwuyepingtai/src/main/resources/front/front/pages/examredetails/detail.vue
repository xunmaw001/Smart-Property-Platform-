	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.examredetailsId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('examredetails', this.examredetailsId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.examredetailsPhoto ? this.detail.examredetailsPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('examredetailsId', _this.examredetailsId);
                                // _this.$utils.jump('/pages/examredetailsYuyue/add-or-update');

								await _this.$api.add('examredetailsYuyue', {
									yonghuId: _this.user.id,
									examredetailsId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/examredetailsYuyue/list');
							}
						}
					});
				},
