	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.bianminName" type="text" placeholder="便民服务名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('bianmin','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('bianmin','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='bianminTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in bianminTypesList" :key="index" :class="bianminTypes===item.codeIndex?'tabActive':''"
                                  @tap="bianminTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.bianminPhoto?baseUrl+product.bianminPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="flex: 1;">
                <view
                        ...
                        class="title hide1">{{product.bianminName}}
                </view>
                <view v-if="product.bianminNewMoney"
                      ...
                      class="title hide1">￥{{product.bianminNewMoney}}
                </view>
                <view v-else-if="product.bianminValue"
                      :style='{"padding":"10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title hide1">{{product.bianminValue}}
                </view>
                <view style="display: flex;justify-content: space-between;padding: 0 10rpx">
                    <text v-if="isAuth('bianmin','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('bianmin','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 便民服务类型 字段 字典表查询方法
            async bianminTypesSelect(){
                let bianminTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'bianmin_types',
                });
                bianminTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.bianminTypesList = bianminTypes.data.list
            },
            //当前表的 便民服务类型字段类别搜索
			bianminTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.bianminTypes != 0) {
                    params.bianminTypes = this.bianminTypes
                }
                let _this = this
                let res = await this.$api.list(`bianmin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('bianmin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`bianmin`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.bianminName) {
                    searchForm['bianminName'] = this.searchForm.bianminName
                }

                let res = await this.$api.list(`bianmin`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.bianminContent!=null){
                        item.bianminContent = item.bianminContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
