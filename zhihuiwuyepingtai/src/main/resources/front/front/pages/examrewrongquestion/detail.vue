	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.examrewrongquestionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('examrewrongquestion', this.examrewrongquestionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.examrewrongquestionPhoto ? this.detail.examrewrongquestionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('examrewrongquestionId', _this.examrewrongquestionId);
                                // _this.$utils.jump('/pages/examrewrongquestionYuyue/add-or-update');

								await _this.$api.add('examrewrongquestionYuyue', {
									yonghuId: _this.user.id,
									examrewrongquestionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/examrewrongquestionYuyue/list');
							}
						}
					});
				},
