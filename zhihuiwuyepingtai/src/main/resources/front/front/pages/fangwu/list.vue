	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.fangwuName" type="text" placeholder="房屋名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('fangwu','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('fangwu','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='fangwuTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in fangwuTypesList" :key="index" :class="fangwuTypes===item.codeIndex?'tabActive':''"
                                  @tap="fangwuTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.fangwuPhoto?baseUrl+product.fangwuPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="flex: 1;">
                <view
                        ...
                        class="title hide1">{{product.fangwuName}}
                </view>
                <view v-if="product.fangwuNewMoney"
                      ...
                      class="title hide1">￥{{product.fangwuNewMoney}}
                </view>
                <view v-else-if="product.fangwuValue"
                      :style='{"padding":"10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title hide1">{{product.fangwuValue}}
                </view>
                <view style="display: flex;justify-content: space-between;padding: 0 10rpx">
                    <text v-if="isAuth('fangwu','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('fangwu','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 房屋类型 字段 字典表查询方法
            async fangwuTypesSelect(){
                let fangwuTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'fangwu_types',
                });
                fangwuTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.fangwuTypesList = fangwuTypes.data.list
            },
            //当前表的 房屋类型字段类别搜索
			fangwuTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.fangwuTypes != 0) {
                    params.fangwuTypes = this.fangwuTypes
                }
                let _this = this
                let res = await this.$api.list(`fangwu`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('fangwu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`fangwu`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.fangwuName) {
                    searchForm['fangwuName'] = this.searchForm.fangwuName
                }
				if (this.searchForm.fangwuAddress) {
                    searchForm['fangwuAddress'] = this.searchForm.fangwuAddress
                }

                let res = await this.$api.list(`fangwu`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.fangwuContent!=null){
                        item.fangwuContent = item.fangwuContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
