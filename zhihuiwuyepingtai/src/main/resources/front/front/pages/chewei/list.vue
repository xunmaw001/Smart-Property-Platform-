	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.cheweiName" type="text" placeholder="车位名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('chewei','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('chewei','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='cheweiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in cheweiTypesList" :key="index" :class="cheweiTypes===item.codeIndex?'tabActive':''"
                                  @tap="cheweiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.cheweiPhoto?baseUrl+product.cheweiPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="flex: 1;">
                <view
                        ...
                        class="title hide1">{{product.cheweiName}}
                </view>
                <view v-if="product.cheweiNewMoney"
                      ...
                      class="title hide1">￥{{product.cheweiNewMoney}}
                </view>
                <view v-else-if="product.cheweiValue"
                      :style='{"padding":"10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title hide1">{{product.cheweiValue}}
                </view>
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 车位类型 字段 字典表查询方法
            async cheweiTypesSelect(){
                let cheweiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'chewei_types',
                });
                cheweiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.cheweiTypesList = cheweiTypes.data.list
            },
            //当前表的 车位类型字段类别搜索
			cheweiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.cheweiTypes != 0) {
                    params.cheweiTypes = this.cheweiTypes
                }
                let _this = this
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`chewei`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
