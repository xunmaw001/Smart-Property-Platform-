	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 0px rgba(0,0,0,.6) inset","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"10rpx 2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.yuangongName" type="text" placeholder="员工姓名"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('yuangong','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('yuangong','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.yuangongName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('yuangong','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.yuangongPhone" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">员工手机号</text>
                     <text style="width: 70%">
                         <text v-if="item.yuangongPhone != null">{{item.yuangongPhone}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                 <view v-show="item.yuangongIdNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">员工身份证号</text>
                     <text style="width: 70%">
                         <text v-if="item.yuangongIdNumber != null">{{item.yuangongIdNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
            ...
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('yuangong','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('yuangong','审核') && item.yuangongYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`yuangong`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.yuangongName) {
                    searchForm['yuangongName'] = this.searchForm.yuangongName
                }
				if (this.searchForm.yuangongPhone) {
                    searchForm['yuangongPhone'] = this.searchForm.yuangongPhone
                }
				if (this.searchForm.yuangongIdNumber) {
                    searchForm['yuangongIdNumber'] = this.searchForm.yuangongIdNumber
                }

                let res = await this.$api.list(`yuangong`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
