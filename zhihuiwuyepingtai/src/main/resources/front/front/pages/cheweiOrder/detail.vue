	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('cheweiOrder', this.cheweiOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.cheweiOrderPhoto ? this.detail.cheweiOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('cheweiOrderYuyue', {
									yonghuId: _this.user.id,
									cheweiOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/cheweiOrderYuyue/list');
							}
						}
					});
				},
