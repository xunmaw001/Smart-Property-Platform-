	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.examquestionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('examquestion', this.examquestionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.examquestionPhoto ? this.detail.examquestionPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('examquestionId', _this.examquestionId);
                                // _this.$utils.jump('/pages/examquestionYuyue/add-or-update');

								await _this.$api.add('examquestionYuyue', {
									yonghuId: _this.user.id,
									examquestionId: _this.detail.id,
								});
								_this.$utils.msg('操作成功');
								_this.$utils.jump('/pages/examquestionYuyue/list');
							}
						}
					});
				},
