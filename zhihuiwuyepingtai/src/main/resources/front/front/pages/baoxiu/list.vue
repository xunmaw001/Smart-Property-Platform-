	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input
					...
					v-model="searchForm.baoxiuName" type="text" placeholder="报修名称"></input>
			</view>
			...
			<view class="action">
				...
				<view v-if="isAuth('baoxiu','新增')" @click="onAddTap()"
					:style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
					新增
				</view>
				<view v-if="isAuth('baoxiu','报表')" @tap="openChartDialog"
					:style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
					报表
				</view>
		...
		<view style="display: flex;flex-wrap: wrap">
			<uni-drawer ref="showRight" mode="left" :maskClick="true" :width="100" :mask-click="true">
				<scroll-view scroll-y="true" class="tabView"
					:style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
					<view style="display: flex;">
						<view style="display: flex;flex-direction: column;width: 100%;">
							<view :style='baoxiuTypes===item.codeIndex?sactiveItem:sitem' class="tab"
								v-for="(item,index) in baoxiuTypesList" :key="index"
								:class="baoxiuTypes===item.codeIndex?'tabActive':''"
								@tap="baoxiuTypesCategoryClick(item.codeIndex)">
								{{item.indexName}}
							</view>
						</view>
					</view>
			...
			<view
				...
				<view class="list b-b" v-for="(item, index) in list" :key="index"
					...
					<view
						...
						<h4>{{ item.baoxiuName }}</h4>
						<view>
							<uni-icons v-show="isAuth('baoxiu','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
								size="20"></uni-icons>
						</view>
					</view>
					<!-- 级联表 -->
					<view v-show="item.yonghuName"
					...
					<!-- 当前表 -->
					<view v-show="item.baoxiuValue"
						style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
						<text style="width: 25%;margin-right: 5%;">报修类型</text>
						<text style="width: 70%">{{item.baoxiuValue}}</text>
					</view>
					<view v-show="item.baoxiuZhuangtaiValue"
						style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
						<text style="width: 25%;margin-right: 5%;">报修状态</text>
						<text style="width: 70%">{{item.baoxiuZhuangtaiValue}}</text>
					</view>
					<view v-show="item.baoxiuContent"
						style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
						<text style="width: 25%;margin-right: 5%;">报修详情</text>
						<view style="width: 70%" v-html="item.baoxiuContent"></view>
					</view>
					<view
						...
						<uni-icons v-show="isAuth('baoxiu','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
						</uni-icons>
						<!--<view v-if="isAuth('baoxiu','审核') && item.baoxiuYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
			...
			//当前表的 报修类型 字段 字典表查询方法
			async baoxiuTypesSelect() {
				let baoxiuTypes = await this.$api.page('dictionary', {
					page: 1,
					limit: 100,
				...
				this.baoxiuTypesList = baoxiuTypes.data.list
			},
			//当前表的 报修类型字段类别搜索
			baoxiuTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
				...
				if (this.baoxiuTypes != 0) {
					params.baoxiuTypes = this.baoxiuTypes
				}
				let _this = this
				let res = await this.$api.list(`baoxiu`, params);
				res.data.list.forEach(function(item, index) {
					if (item.baoxiuContent != null) {
						item.baoxiuContent = item.baoxiuContent.replace("<img", "<img  class=\"daxiao\"")
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('baoxiu', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update(`baoxiu`, data);
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
			...
			async search() {
				...
				if (this.searchForm.baoxiuName) {
					searchForm['baoxiuName'] = this.searchForm.baoxiuName
				}

				let res = await this.$api.list(`baoxiu`, searchForm);
				let _this = this
				res.data.list.forEach(function(item, index) {
					if (item.baoxiuContent != null) {
						item.baoxiuContent = item.baoxiuContent.replace("<img", "<img  class=\"daxiao\"")
