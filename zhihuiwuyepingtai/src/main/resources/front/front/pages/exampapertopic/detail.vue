	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.exampapertopicId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('exampapertopic', this.exampapertopicId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.exampapertopicPhoto ? this.detail.exampapertopicPhoto.split(",") : [];
				},
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
                                ...
                                // uni.setStorageSync('exampapertopicId', _this.exampapertopicId);
                                // _this.$utils.jump('/pages/exampapertopicYuyue/add-or-update');

								await _this.$api.add('exampapertopicYuyue', {
