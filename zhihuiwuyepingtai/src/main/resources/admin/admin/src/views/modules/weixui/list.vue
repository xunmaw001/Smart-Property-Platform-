      ...
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '员工姓名' : ''">
            <el-input prefix-icon="el-icon-search" v-model="searchForm.yuangongName" placeholder="员工姓名"
                      clearable></el-input>
          </el-form-item>

          <el-form-item :label="contents.inputTitle == 1 ? '员工手机号' : ''">
            <el-input prefix-icon="el-icon-search" v-model="searchForm.yuangongPhone" placeholder="员工手机号"
                      clearable></el-input>
          </el-form-item>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            ...
            <a style="text-decoration:none" class="el-button el-button--success"
               v-if="isAuth('weixui','导入导出')"
               icon="el-icon-download"
               href="http://localhost:8080/zhihuiwuyepingtai/upload/weixuiMuBan.xls"
            >批量导入维修指派数据模板</a>
            &nbsp;
            <el-upload
                v-if="isAuth('weixui','导入导出')"
                ...
                :action="getActionUrl"
                :on-success="weixuiUploadSuccess"
                :on-error="weixuiUploadError"
            ...
            &nbsp;
            <!-- 导出excel -->
            <download-excel v-if="isAuth('weixui','导入导出')" style="display: inline-block"
                            class="export-excel-wrapper" :data="dataList" :fields="json_fields" name="weixui.xls">
              <!-- 导出excel -->
              <el-button
                  type="success"
                  icon="el-icon-download"
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
                           ...
                           label="备注">
            <template slot-scope="scope">
                            <span v-if="scope.row.weixuiText != null &&scope.row.weixuiText.length>10">
                                {{ scope.row.weixuiText.slice(0, 10) }}...
                            </span>
              <span v-else>
                                {{ scope.row.weixuiText }}
                            </span>
            </template>
          </el-table-column>
          ...
          <el-table-column width="300" :align="contents.tableAlign"
            ...
            <template slot-scope="scope">
              ...
              <el-button v-if="isAuth('weixui','查看')" type="success" icon="el-icon-tickets" size="mini"
                         @click="addOrUpdateHandler(scope.row.id,'info')">详情
              </el-button>
              <el-button v-if="isAuth('weixui','修改')" type="primary" icon="el-icon-edit" size="mini"
                         @click="addOrUpdateHandler(scope.row.id)">修改
              </el-button>
              <el-button v-if="isAuth('weixui','删除')" type="danger" icon="el-icon-delete" size="mini"
                         @click="deleteHandler(scope.row.id)">删除
              </el-button>
              <el-button v-if="isAuth('weixui','已维修') && scope.row.baoxiuZhuangtaiTypes !=1 " type="primary" icon="el-icon-edit" size="mini"
                         @click="weixui(scope.row.id)">已维修
              </el-button>
            </template>
          </el-table-column>
        </el-table>
  ...
  methods: {
    chartDialog() {
      ...
      let params = {
        ...
        thisTable: {//当前表
          tableName: 'weixui',//当前表表名,
          sumColum: 'weixui_number', //求和字段
          date: 'insert_time',//分组日期字段
          // string : 'weixui_name',//分组字符串字段
          // types : 'weixui_types',//分组下拉框字段
        },
        // joinTable : {//级联表（可以不存在）
    ...
    getDataList() {
      ...
      params['weixuiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
    ...
    // 导入功能上传文件成功后调用导入方法
    weixuiUploadSuccess(data) {
      let _this = this;
      _this.$http({
        url: "weixui/batchInsert?fileName=" + data.file,
        method: "get"
      }).then(({data}) => {
        if (data && data.code === 0) {
          _this.$message({
